...
BOOL FindPattren(HANDLE Proc, BYTE* bSearchData, int nSearchSize, DWORD_PTR dwStartAddr, DWORD_PTR dwEndAddr, BOOL bIsCurrProcess, int iSearchMode, std::vector<DWORD_PTR>& vRet)
{
    ...
    for (it = m_vMemoryRegion.begin(); it != m_vMemoryRegion.end(); it++)
    {
        ...
        if (iSearchMode == 0)
        {
            ...
            int iOffset = MemFind(pCurrMemoryData, (int)dwNumberOfBytesRead, bSearchData, nSearchSize);
            while (iOffset != -1)
            {
                dwOffset += iOffset;
                vRet.push_back(dwOffset + mData.dwBaseAddr);
                dwOffset += nSearchSize;
                iOffset = MemFind(pCurrMemoryData + dwOffset, (int)(dwNumberOfBytesRead - dwOffset - nSearchSize), bSearchData, nSearchSize);
            }
        }
        else if (iSearchMode == 1)
        {
            DWORD_PTR dwOffset = 0;
            int iOffset = SundaySearch(pCurrMemoryData, (int)dwNumberOfBytesRead, bSearchData, nSearchSize);
...
{
    SIZE_T Read;
    BOOL Ret = ReadProcessMemory(hProc, pAddress, pOut, MaxLength, &Read);
    if (!Ret || !Read)
    {
        return 0;
    }
